...
2. `rebuild_nodes` tears down all fifty rows and builds fifty more, and now that `refresh_node_list` redraws the sidebar whenever the newest page has moved, that happens on any poll where the order changed rather than only on `r`. Measured on this Mac with fifty rows drawn: 36ms to refresh them in place, 135ms median and 251ms worst to rebuild them — roughly 3.7x, and the Pi will feel it more. RxOnly does not pay this because `update_nodes_list` (nodes.js:159) reuses each `li` it already has by node id and re-appends it in the fresh order. The console can do the same: `NodeItem` already knows its `node_id` and `set_node` already refreshes one in place, so a reorder could move existing widgets rather than replace them, leaving construction for genuinely new nodes. What stops it being a two-line change is `striped()`, which sets the alternating row class from the index, so anything moved has to be restriped.
